...
export function emptyTotals(): UsageTotals {
  return { input: 0, output: 0, cacheRead: 0, cacheWrite: 0, reasoning: 0, total: 0, cost: 0, calls: 0 };
}
...
function toUsageRecord(sessionFile: string, entryId: string, timestamp: string, provider: string, model: string, usage: {
  input?: unknown;
  output?: unknown;
  cacheRead?: unknown;
  cacheWrite?: unknown;
...
export function recordFromEntry(sessionFile: string, entry: {
  type?: unknown;
  id?: unknown;
  timestamp?: unknown;
  message?: {
  ...
  if (entry?.type !== "message" || !entry.message || entry.message.role !== "assistant") return null;
  const usage = entry.message.usage;
  if (!usage || typeof usage !== "object") return null;
  const entryId = typeof entry.id === "string" ? entry.id : "";
  if (!entryId) return null;
  const timestamp = typeof entry.message.timestamp === "string"
...
function ensureLoaded(): void {
  ...
  for (const line of content.split("\n")) {
    ...
    try {
      ...
      if (rec && typeof rec.id === "string" && rec.id) state.records.set(rec.id, rec);
    } catch {
 ...
 */
export function recordUsageFromEntry(sessionFile: string, entry: unknown): boolean {
  const rec = recordFromEntry(sessionFile, entry as Parameters<typeof recordFromEntry>[1]);
...
export function recordUsage(record: UsageRecord): boolean {
  ...
  try {
    ...
    console.error("[pi-web] failed to persist usage record:", err instanceof Error ? err.message : err);
  }
  return true;
}
  ...
  state.backfillPromise = (async (): Promise<number> => {
    ...
    try {
      ...
      for (const cwdDir of cwdDirs) {
        ...
        let files: string[];
        try {
          files = readdirSync(sessionDirPath).filter((f) => f.endsWith(".jsonl"));
        } catch {
          continue;
        }
        for (const file of files) {
          ...
          for (const line of content.split("\n")) {
            ...
            let entry: {
              ...
              id?: unknown;
              timestamp?: unknown;
              message?: { role?: unknown; provider?: unknown; model?: unknown; timestamp?: unknown; usage?: unknown } | null;
            };
            try {
...
export async function getUsageSummary(options: { days?: number; forceBackfill?: boolean } = {}): Promise<UsageSummaryResponse> {
  ...
  const byModel = [...modelMap.values()].sort((a, b) => b.total - a.total).slice(0, 12);
...
export async function getUsageDetail(day: string): Promise<UsageDetailResponse> {
  await backfillUsage();
  const state = getState();
