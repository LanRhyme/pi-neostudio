...
export function ChatMinimap({
	...
	// 悬停地图时自动链式加载压缩前的更早对话（每页 50 轮），直到完整显示
	const autoLoadStoppedRef = useRef(false);
	const autoLoadLastLenRef = useRef<number | null>(null);
	useEffect(() => {
		if (!minimapHovered || earlierLoading || !earlierHasMore) return;
		if (!onLoadEarlier) return;
		if (autoLoadStoppedRef.current) return;
		if (messages.length >= 1500) return; // 安全上限，超出后靠 ↑ 按钮手动加载
		// 上次自动加载后消息数未增长 → 加载失败，停止自动链避免无限重试
		if (
			autoLoadLastLenRef.current !== null &&
			messages.length === autoLoadLastLenRef.current
		) {
			autoLoadStoppedRef.current = true;
			return;
		}
		autoLoadLastLenRef.current = messages.length;
		const t = setTimeout(() => onLoadEarlier(), 150);
						...
						<div
							style={{
								...
								boxShadow: isActive
									...
									: "none",
								transition:
									"transform 0.18s cubic-bezier(0.16,1,0.3,1), background 0.18s, border-color 0.18s, box-shadow 0.18s",
