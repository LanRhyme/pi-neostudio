...
export const ChatWindow = memo(function ChatWindow({
	...
	const desktopNotificationsRef = useRef(uiSettings.desktopNotifications);
	desktopNotificationsRef.current = uiSettings.desktopNotifications;
	const sessionNameRef = useRef(session?.name ?? session?.firstMessage ?? "");
	sessionNameRef.current = session?.name ?? session?.firstMessage ?? "";
	const notifyDoneRef = useRef<() => void>(() => {});
	notifyDoneRef.current = () => {
		...
		try {
			...
			new Notification(t("notification.done"), {
				body: name
					? t("notification.body", { title: name })
					: t("notification.finished"),
				silent: true,
			});
		} catch {
			// Notification constructor can throw in some environments — ignore.
		}
	};
					...
					<div className="relative flex min-w-0 flex-1 overflow-hidden">
						...
						>
							...
							>
								...
								>
									...
									<div
										style={{ height: "var(--chat-input-height, 200px)" }}
										aria-hidden="true"
									/>
								</div>
							</div>
						...
						<div
							style={{
								...
								background:
									"linear-gradient(to bottom, transparent 0%, color-mix(in srgb, var(--bg) 70%, transparent) 28%, color-mix(in srgb, var(--bg) 96%, transparent) 60%, var(--bg) 100%)",
							}}
						>
							<div style={{ maxWidth: 820, margin: "0 auto" }}>
