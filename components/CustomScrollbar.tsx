 ...
 */
export function CustomScrollbar({ containerRef, right = 0, width = 8, zIndex = 40 }: CustomScrollbarProps) {
  ...
  const trackClick = (e: React.MouseEvent) => {
    ...
    const ratio = (y - thumb.height / 2) / Math.max(1, el.clientHeight - thumb.height);
    ...
    >
      ...
      <div
        ...
        style={{
          ...
          left: 0,
          width: "100%",
          background: hover ? "color-mix(in srgb, var(--border) 50%, transparent)" : "transparent",
      ...
      <div
        onMouseDown={(e) => {
          ...
          const onMove = (ev: MouseEvent) => {
            ...
            if (!d || !c || !t) return;
            const ratio = (c.scrollHeight - c.clientHeight) / Math.max(1, c.clientHeight - t.height);
