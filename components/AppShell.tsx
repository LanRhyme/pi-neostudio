...
export function AppShell() {
		...
		() =>
			typeof window === "undefined"
				...
				: getSidebarMaxWidth({
						...
						rightPanelOpen: uiPanelStore.isRightPanelOpen(),
	...
	const getResponsiveRightPanelMaxWidth = useCallback(
		() =>
			typeof window === "undefined"
				...
				: getRightPanelMaxWidth({
						viewportWidth: window.innerWidth,
						sidebarOpen: uiPanelStore.isSidebarOpen(),
	...
	const handleSystemPromptLoaderChange = useCallback(
	...
	const [externalLinkToOpen, setExternalLinkToOpen] = useState<string | null>(
		null,
	);
	...
	const handleCwdChange = useCallback(
		(cwd: string | null, projectRoot?: string | null) => {
			...
			uiPanelStore.setRightPanelOpen(false);
	...
	const handleSelectSession = useCallback(
		(session: SessionInfo, isRestore = false) => {
			...
			if (isMobile && !isRestore) uiPanelStore.setSidebarOpen(false);
			if (isRestore) {
	...
	const handleOpenFile = useCallback(
		(
			...
			uiPanelStore.setRightPanelOpen(true);
	...
	const handleCloseFileTab = useCallback(
		(tabId: string) => {
			setFileTabs((prev) => {
				...
				if (next.length === 0) uiPanelStore.setRightPanelOpen(false);
				return next;
			});
    ...
    `}</style>
				...
				>
					...
					<SidebarPanel
						...
						separatorTitle={`${translate("layout.resizeSidebar")}: ${translate("layout.resizeHint")}`}
					>
						{sidebarContent}
					</SidebarPanel>
										...
										return (
											<button
												...
												style={{
													...
													whiteSpace: "nowrap",
													transition:
														"color 0.1s, background 0.1s, opacity 0.1s",
												}}
												onMouseEnter={(e) => {
													if (disabled) return;
						...
						>
							{activeFileTab?.filePath ? (
								activeFileTab.filePath.startsWith("browser://") ? (
									<SimpleBrowser
										initialUrl={activeFileTab.filePath.replace(
											"browser://",
											"",
										)}
										onInsertText={handleInsertText}
