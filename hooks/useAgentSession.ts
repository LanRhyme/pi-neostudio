...
export function useAgentSession(opts: UseAgentSessionOptions) {
	...
	const deriveEarlierBefore = (msgs: AgentMessage[]): string | null => {
		const first = msgs[0];
		if (!first || first.role !== "custom" || first.customType !== "compaction")
			return null;
	...
	const applyContextMessages = useCallback(
		(newMessages: AgentMessage[], newEntryIds: string[]) => {
	...
	const loadEarlierHistory = useCallback(async () => {
		...
		try {
			...
			const res = await fetch(
				`/api/sessions/${encodeURIComponent(sid)}/earlier?${params}`,
			);
	...
	const loadSystemPrompt = useCallback(async () => {
		...
		const state = await sendAgentCommand<{ systemPrompt?: string | null }>(
			sid,
			{ type: "get_state" },
		);
		if (sessionIdRef.current !== sid) return;
			...
			switch (event.type) {
				...
				case "message_update": {
					...
					if (msg) {
						...
						streamPendingMsgRef.current = normalizeToolCalls(
							msg as AgentMessage,
						);
						if (streamFrameRef.current === null) {
