...
- npm package: `pi-neostudio`
- GitHub Release: `LanRhyme/pi-neostudio`
...
- npm is authenticated as an account that can publish `pi-neostudio` (run `npm login` if not)
...
- It intentionally runs a production build
  Do not run `next build` during normal development, release work is the exception
- If `npm view pi-neostudio version` briefly shows the previous version, check the exact version instead

```bash
npm view pi-neostudio@<version> version --registry https://registry.npmjs.org/
npm view pi-neostudio versions --json --registry https://registry.npmjs.org/
```

## 3. Commit the Version Bump

Replace `<version>` with the new package version, for example `0.8.7`
...
gh release view v<version> --repo LanRhyme/pi-neostudio
```
...
Write the release notes from those commits, not from memory
Include both Chinese and English sections
Keep commit hashes next to each item when useful
...
- Published npm package `pi-neostudio@<version>`
```
...
```bash
gh release edit v<version> --repo LanRhyme/pi-neostudio --notes-file - <<'EOF'
## 中文
